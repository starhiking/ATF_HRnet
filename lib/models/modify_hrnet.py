...
class HRNET_BACKBONE(nn.Module):
    ...
    def __init__(self, config):
        ...
        global HRNET_final_inp_channels 
        HRNET_final_inp_channels = sum(pre_stage_channels)
...
class HRNET_HEAD(nn.Module):
    def __init__(self,num_landmark):
        super(HRNET_HEAD,self).__init__()
        self.head = nn.Sequential(
                nn.Conv2d(
                    in_channels=HRNET_final_inp_channels,
                    out_channels=HRNET_final_inp_channels,
                    kernel_size=1,
                    stride=1,
                    padding=0),
                BatchNorm2d(HRNET_final_inp_channels, momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    in_channels=HRNET_final_inp_channels,
                    out_channels=num_landmark,
                    kernel_size=1,
                    stride=1,
                    padding=0)
            )
...
def get_face_alignment_nets(config,data_types,aug_sigma=1.5):
    ...
    heads = {}
    heads[str(config.MODEL.NUM_JOINTS)] = HRNET_HEAD(config.MODEL.NUM_JOINTS)
    heads[str(config.MODEL.NUM_JOINTS)].init_weights(pretrained=pretrained)
    ...
    return backbone , heads , aux_configs
...
if __name__ == "__main__":
    ...
    update_config(config,"experiments/wflw/face_alignment_wflw_hrnet_w18.yaml")

    backbone , heads , aux_configs = get_face_alignment_nets(config,"300w","aflw","cofw")
    backbone = backbone.eval()
    for key in heads.keys():
        heads[key] = heads[key].eval()

    net_input = torch.randn(10,3,256,256)        
    start = time.time()

    for i in range(1):
        feature_map = backbone(net_input)
        for key in heads.keys():
            print(key,heads[key](feature_map).shape )

    print("per image process time : {:.4f}".format((time.time() - start)/1))
    
    from thop import profile,clever_format
    flops, params = profile(backbone,inputs=(net_input, ))    
